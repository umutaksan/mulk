...
SELECT 
  ...
  m.priority,
  m.status,
  m.price::decimal(10,2)
FROM (
  VALUES 
    ('Marbella Old Town', 'AC Maintenance', 'Regular maintenance check for all AC units', '2025-03-15', 'medium', 'pending', 150.00),
    ('Jardines Tropicales-Puerto Banús', 'Pool Cleaning', 'Weekly pool maintenance and chemical balance check', '2025-03-10', 'high', 'in-progress', 80.00)
) as m(property_name, title, description, due_date, priority, status, price)
JOIN property_ids p ON p.name = m.property_name;